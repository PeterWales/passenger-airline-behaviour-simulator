            ...
            if (
                ...
                if (
                    ...
                    if not itineraries[
                        ...
                        test_itin_out = itineraries.loc[out_itin_mask]
                        test_itin_in = itineraries.loc[in_itin_mask]
...
def reassign_ac_to_new_route(
    ...
    """
    Reassign an aircraft to a new route and make the necessary adjustments to all dataframes.
    ...
    reassign_itin_in : pd.Series
        Series of inbound itinerary data for the itinerary the aircraft is being removed from
    reassign_ac : pd.Series
        Series of aircraft data for the aircraft being reassigned
    addnl_flights_per_year : int
        Number of flights per year the aircraft can make on the new itinerary
    ...
    return airline_routes, airline_fleets, city_data, city_pair_data
...
def update_profit_tracker(
    ...
    rtn_flt_df : pd.DataFrame
    """
    # check if itinerary already exists in rtn_flt_df
    if rtn_flt_df[
